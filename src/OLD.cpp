...
/*
    ...
    1. parameterize individual methods
    2. finally tackle lot polygons
    3. then move on to building generation
    4. fix elevation
    5. generate ground poly
 */
...
vector<shared_ptr<Road>> ofxProceduralCity::globalGoals(shared_ptr<Road> a){
    ...
    for(int i = 0; i < max_goals; i++){
        ...
        bool accepted = true;
        if(a->prev != nullptr){
            bool pattern_check = rightAngleGoal(a, next_node);
            bool pop_check = populationGoal(a, next_node);
...
bool ofxProceduralCity::rightAngleGoal(shared_ptr<Road> prev, ofVec3f &end){
...
bool ofxProceduralCity::populationGoal(shared_ptr<Road> prev, ofVec3f &end){
    if(prev->prev == nullptr){ return true; }
